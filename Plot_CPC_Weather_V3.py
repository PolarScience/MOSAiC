...
start_sample = pd.to_datetime('2019-12-27 00:00')
end_sample = pd.to_datetime('2019-12-29 00:00')
...
## This means that 
#dt['threshold']=  
dt['cutoff_marker'] = np.where(dt['grad_mean']>dt['cpc_mean']*0.04, 1,0)   #this is wrong. 
...
df_1min['line']= df_1min['counts']*0.04
...
#%% Histogram of standardized gradients
## THIS PLOT IS NOT USEFUL?
# fig, ax = plt.subplots()
# ax.hist(scaled_df['grad_counts'],bins = 100, log = True, alpha = 0.3, label = 'standardized gradients of counts')
# # ax.hist(dt['cpc_good'].values, bins=100, log = True, range = (0,5000), color='orange', alpha = 0.3, label='CPC_cleaned' )
# # ax1.hist(flag['cpc_good_median'].values, bins=100, log = True, range = (0,5000), color='yellow', alpha = 0.3, label='CPC_cleaned_median' )
# ax.set_title('Comparison of cleaned and uncleaned data. '+startdate.strftime('%Y/%m/%d %Hh')+' - '+enddate.strftime('%Y/%m/%d %Hh'), fontsize = fs)
# ax.set_ylabel('Frequency', fontsize = fs)
...
# ax1.plot(dt['cpc_bad'], '.',label = 'Bad data', lw= line_width,color=zcol, markersize = ms)
# ax1.plot(dt['cpc_good'], '.',label = 'Good data', lw= line_width, markersize = ms)
...
# fig, ax = plt.subplots()
# ax.scatter(df_1min['counts'],df_1min['norm'],s=s,co)
